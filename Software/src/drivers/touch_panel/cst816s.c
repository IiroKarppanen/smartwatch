...
static int cst816s_process(const struct device *dev)
{
	...
	printk("Event: %u", event);
	printk("Pressed: %u", is_pressed);
	printk("Gesture: %u", output.gesture);
...
static int cst816s_chip_init(const struct device *dev)
{
	...
	if (chip_id != CST816S_CHIP_ID) {
		printk("CST816S wrong chip id: returned 0x%x", chip_id);
		return -ENODEV;
	}
