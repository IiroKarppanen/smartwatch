                        ...
                        const void *buf)
{
    ...
    size_t len = (239 + 1 - x) * (239 + 1 - y) * 16 / 8;
...
int gc9a01_write_data(const struct device *dev, const void *tx_data, size_t tx_len){
    ...
    gc9a01_write_command(dev, GC9A01A_RAMWR);
    r = spi_write_dt(&config->bus, &tx_bufs);
	if (r < 0) {
        printk("SPI WRITE FAILED\n");
		return r;
...
int gc9a01_controller_init(const struct device *dev)
{
    ...
    disp_drv.user_data = (void *)dev;  // dev is the device instance of the GC9A01 display
    //gc9a01_write(dev, 0, 0, NULL, BLACK);
...
static void GC9A01_flush(struct _disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p) {
    uint16_t x, y;
    const struct device *dev = disp_drv->user_data;

    /* Set the frame based on the area to be updated */
    ...
    for (y = area->y1; y <= area->y2; y++) {
        for (x = area->x1; x <= area->x2; x++) {
            /* Assuming color_p is an array of lv_color_t with LV_COLOR_DEPTH bits per pixel */
            uint16_t pixel = LV_COLOR_16_SWAP(color_p->full);
